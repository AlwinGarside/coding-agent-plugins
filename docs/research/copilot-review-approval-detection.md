...
The safe implementation is fail-closed:

1. Use structured GraphQL fields to establish that the expected GitHub bot submitted a review for the current head commit and that no newer review request is pending.
2. Accept `PullRequestReview.state == APPROVED` if GitHub ever emits it for the Copilot bot. GitHub defines that state as a review allowing the pull request to merge.
3. With today's normal Copilot behavior, `state` is `COMMENTED`. Treat that as **not machine-verifiable approval**, not success. Return “needs review” and include the review URL.
4. If uninterrupted automation is more important than strict correctness, use a narrowly scoped compatibility parser for the first Markdown heading in the review body. Allowlist only explicitly verified positive headings and fail closed on every unknown heading. This is more robust than matching `generated no (new) comments`, but it is still an undocumented text dependency.

The current script's structured checks for completion, staleness, and unresolved threads remain useful. They cannot prove approval and should not be described as doing so.
...
Keep the existing exit-code concept, but reserve exit `0` for a genuine structured approval:

```text
identify Copilot by GraphQL Bot node ID
        |
        v
newer request than submitted review? ---- yes ---> still running / needs review
        |
        no
        v
latest review commit != current head? ---- yes ---> stale / needs review
        |
        no
        v
review state == APPROVED? ---------------- yes ---> success
        |
        no
        v
review state == CHANGES_REQUESTED? ------- yes ---> feedback / needs review
        |
        no (COMMENTED)
        v
assessment unavailable as structured data ------> needs review
```

Continue returning unresolved published Copilot threads as feedback because they are actionable. When the review is `COMMENTED` and no such threads exist, return “needs review” with the latest review's URL rather than claiming approval.

This deliberately means that current normal Copilot reviews cannot produce exit `0`. That is the accurate result under GitHub's documented API behavior.

### Compatibility mode (only if automation must continue)

If the workflow must automatically accept Copilot's experimental positive assessment, isolate the unavoidable text dependency:

1. Read only the first Markdown heading from the latest Copilot review body.
2. Compare the complete normalized heading to a configuration allowlist of positive assessments that have been directly observed and accepted by the repository owner.
3. Treat `Not ready to approve`, `Human review recommended`, blank headings, and every unknown future heading as “needs review.”
4. Keep commit freshness, pending-request detection, and unresolved-thread checks as independent requirements.
5. Add fixtures for each observed heading and for unknown headings so a format change fails closed.

Do not infer approval from “comments generated: 0,” the absence of `Suppressed comments`, emoji color alone, or a successful check run. Those are weaker proxies than the assessment heading and have counterexamples in PR #126.

The script and skill should call this an **experimental Copilot assessment**, not a GitHub approval, unless the review state is actually `APPROVED`. If the desired operational rule is merely “Copilot completed a fresh review and has no published unresolved threads,” that can be implemented entirely with structured fields, but it must be named accordingly because it ignores suppressed findings.
